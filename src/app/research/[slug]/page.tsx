...
export default async function ResearchDetailPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
      ...
      <h1 className="mt-4 max-w-4xl font-display text-3xl leading-tight sm:text-5xl lg:text-6xl">
        {area.title}
      </h1>
      ...
      <section className="mt-16">
        ...
        <ol className="mt-8 space-y-4">
          {area.questions.map((question, index) => (
            <li key={question} className="flex gap-4 border-t border-line pt-4">
      ...
      <section className="mt-16">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {area.methods.map((method) => (
            <div key={method.name} className="border border-line p-5">
      ...
      <section className="panel mt-16 p-5 sm:p-8">
        ...
        <Link href="/phd" className="mt-6 inline-block text-sm text-accent">
          Ph.D. Program →
        </Link>
      </section>
